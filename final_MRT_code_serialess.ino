...
long Ultra(int trigger, int echo)			//variable trigger and echo
{
  pinMode(trigger, OUTPUT);
  pinMode(echo, INPUT);
  
  digitalWrite(trigger, 0);						//clear the trigger initially
  delayMicroseconds(2);							
  digitalWrite(trigger, 1);						//make it high for 10millisec
  delayMicroseconds(10);
  digitalWrite(trigger, 0);						
  
  return pulseIn(echo, 1);						//'return pulseIn' is an inbuilt func.
}												//converts digital to pulse signals
...
void loop() {
  ...
  distance_status = 0.01723 * Ultra(15, 4);		//NOTE: float for accuracy
                                              // (15, 4) are the pin nos of the trigger and echo pin resp.
